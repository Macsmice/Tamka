...
CREATE TABLE IF NOT EXISTS `#__things` (
...
(1, 0, 0, 1, 'com_things', 'com_things', '{"core.admin":{"7":1},"core.manage":{"6":1},"core.create":{"3":1},"core.delete":[],"core.edit":{"4":1},"core.edit.state":{"5":1},"core.edit.own":[]}');

/* Update RGT for Component Asset Record */
SET @max_rgt = (SELECT MAX(rgt)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET rgt = @max_rgt WHERE `name` = 'com_things';

/* Update LFT for Component Asset Record */
SET @max_lft = (SELECT MAX(lft)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET lft = @max_lft WHERE `name` = 'com_things';
...
/* Update Category Record for Asset ID */
SET @catid = (SELECT id FROM `#__categories` WHERE `extension` = 'com_things');
UPDATE `#__things` SET catid = @catid;

/** 5. ASSET RECORD FOR CONTENT **/

/* Create Content Asset Record */
INSERT INTO `#__assets` (`level`, `name`, `title`, `rules`)
    SELECT 3 as `level`, CONCAT('com_things.thing', '.', id) as `name`, title as `title`, '{"core.create":[],"core.delete":[],"core.edit":[],"core.edit.state":[],"core.edit.own":[]}' as `rules`
     FROM `#__things`;

/* Update Category Record for Asset ID */
SET @asset_id = (SELECT id FROM `#__assets` WHERE `name` like 'com_things.thing.%');
UPDATE `#__things` SET asset_id = @asset_id;

/* Update Parent ID for Component Asset Record */
SET @parent_id = (SELECT id FROM `#__assets` WHERE `name` = 'com_things');
UPDATE `#__assets` SET parent_id = @parent_id WHERE `name` like 'com_things.thing.%';

/* Update RGT for Component Asset Record */
SET @max_rgt = (SELECT MAX(rgt)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET rgt = @max_rgt WHERE `name` like 'com_things.thing.%';

/* Update LFT for Component Asset Record */
SET @max_lft = (SELECT MAX(lft)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET lft = @max_lft WHERE `name` like 'com_things.thing.%';
...
INSERT INTO `#__molajo_configuration` (`component_option`, `option_id`, `option_value`, `option_value_literal`, `ordering`) VALUES
('com_things', 45, '', '', 0),
('com_things', 45, '__things', '__things', 1);

/* 050 MOLAJO_CONFIG_OPTION_ID_EDIT_LAYOUTS */;
INSERT INTO `#__molajo_configuration` (`component_option`, `option_id`, `option_value`, `option_value_literal`, `ordering`) VALUES
('com_things', 50, '', '', 0),
('com_things', 50, 'thing', 'thing', 1);

/* 060 MOLAJO_CONFIG_OPTION_ID_DEFAULT_LAYOUT */;
INSERT INTO `#__molajo_configuration` (`component_option`, `option_id`, `option_value`, `option_value_literal`, `ordering`) VALUES
('com_things', 60, '', '', 0),
('com_things', 60, 'things', 'things', 1);
