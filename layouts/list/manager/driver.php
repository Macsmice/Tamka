...
$document->addStyleSheet('../media/'.$this->options->get('request.option').'/css/administrator.css');

/** Parameters **/
$this->options = JComponentHelper::getParams($this->options->get('request.option'));

/** list variables **/
$this->getState('pagination.list_order') = $this->queryState->get('list.ordering');
$listOrder = $this->escape($this->queryState->get('list.ordering'));
$listDirn = $this->escape($this->queryState->get('list.direction'));
$this->saveOrder    = $this->getState('pagination.list_order') == 'a.ordering';
