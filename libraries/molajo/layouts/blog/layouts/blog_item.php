...
$canEdit	= $this->row->params->get('access-edit');
...
<?php if ($params->get('show_title')) : ?>
	<h2>
		<?php if ($params->get('link_titles') && $params->get('access-view')) : ?>
			<a href="<?php echo JRoute::_(ContentHelperRoute::getArticleRoute($this->row->slug, $this->row->catid)); ?>">
...
<?php endif; ?>
<?php if ($params->get('show_parent_category') && $this->row->parent_id != 1) : ?>
		<dd class="parent-category-name">
			<?php $title = $this->escape($this->row->parent_title);
				$url = '<a href="' . JRoute::_(ContentHelperRoute::getCategoryRoute($this->row->parent_id)) . '">' . $title . '</a>'; ?>
...
<?php if ($params->get('show_category')) : ?>
		<dd class="category-name">
			<?php $title = $this->escape($this->row->category_title);
					$url = '<a href="' . JRoute::_(ContentHelperRoute::getCategoryRoute($this->row->catid)) . '">' . $title . '</a>'; ?>
...
<?php if ($params->get('show_readmore') && $this->row->readmore) :
	if ($params->get('access-view')) :
		$link = JRoute::_(ContentHelperRoute::getArticleRoute($this->row->slug, $this->row->catid));
	else :
		$menu = JFactory::getApplication()->getMenu();
		$active = $menu->getActive();
		$itemId = $active->id;
		$link1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $itemId);
		$returnURL = JRoute::_(ContentHelperRoute::getArticleRoute($this->row->slug));
		$link = new JURI($link1);
